...
const CIPHERGUARD_GETTING_STARTED_URL = "https://www.cipherguard.com/start";
...
class ExtQuickAccess extends React.Component {
  ...
  async checkPluginIsConfigured() {
    ...
    if (!isConfigured) {
      browser.tabs.create({url: CIPHERGUARD_GETTING_STARTED_URL});
      window.close();
    }
  }
   ...
   */
  async getAccount() {
    const accountDto = await this.state.port.request("cipherguard.account.get");
