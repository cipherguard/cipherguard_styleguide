...
describe("ResourceViewPage", () => {
  ...
  describe('As LU, I should preview the secret.', () => {
    ...
    it('As LU, I should see username, URI of a resource', async() => {
      ...
      expect(page.username.textContent).toStrictEqual("admin@cipherguard.com");
      expect(page.uri.textContent).toStrictEqual("https://cipherguard.com");
    });
  ...
  describe('As LU, I should copy the secret.', () => {
    it('As LU, I should be able to copy the secret password of resource by clicking on the password', async() => {
      ...
      expect(props.context.port.request).toHaveBeenCalledWith('cipherguard.secret.find-by-resource-id', props.context.storage.local.get(["resources"]).resources[0].id);
