...
describe("ResourceCreatePage", () => {
  describe("Form initialization", () => {
    it("should intialize the name and uri input fields with the active tab metadata", async() => {
      ...
      props.context.port.addRequestListener("cipherguard.quickaccess.prepare-resource", async() => expectedData);
  ...
  describe("Form submission", () => {
    it("should create a new password v4 on submit", async() => {
      ...
      const expectedResourceDto = {
        ...
        metadata: {
          resource_type_id: TEST_RESOURCE_TYPE_PASSWORD_AND_DESCRIPTION,
          name: "Cipherguard Browser Extension Test",
          uris: ["https://cipherguard-browser-extension/test"],
      ...
      props.context.port.addRequestListener("cipherguard.quickaccess.prepare-resource", () => ({
      ...
      props.context.port.addRequestListener("cipherguard.resources.create", (resourceDto, secretDto) => {
    ...
    it("should ask for password creation confirmation if the entropy is too low", async() => {
      ...
      props.prepareResourceContext.consumeLastGeneratedPassword.mockImplementation(() => null);
      props.context.port.addRequestListener("cipherguard.quickaccess.prepare-resource", async() => preparedResource);
      props.context.port.addRequestListener("cipherguard.secrets.powned-password", async() => 0);
      ...
      const expectedPageProps = {
        resourceName: "Cipherguard Browser Extension Test",
    ...
    it("should ask for password creation confirmation if the passphrase is found in a data breach", async() => {
      ...
      props.prepareResourceContext.consumeLastGeneratedPassword.mockImplementation(() => null);
      props.context.port.addRequestListener("cipherguard.quickaccess.prepare-resource", async() => preparedResource);
      props.context.port.addRequestListener("cipherguard.secrets.powned-password", async() => 3);
    ...
    it("should create a new password on submit when the dictionary service is unreachable", async() => {
      ...
      props.context.port.addRequestListener("cipherguard.quickaccess.prepare-resource", async() => {
      ...
      props.context.port.addRequestListener("cipherguard.secrets.powned-password", async password => {
        expect(password).toStrictEqual(expectedSecretDto.password);
        throw new Error();
      });

      props.context.port.addRequestListener("cipherguard.resources.create", async(resourceDto, secretDto) => {
        expect(resourceDto).toStrictEqual(expectedResourceDto);
        expect(secretDto).toStrictEqual(expectedSecretDto);
        return defaultResourceDto();
      });
  ...
  describe("Form validation", () => {
    ...
    it("should display the error from the API", async() => {
      ...
      props.passwordPoliciesContext.shouldRunDictionaryCheck.mockImplementation(() => false);
      props.context.port.addRequestListener("cipherguard.quickaccess.prepare-resource", () => null);
    ...
    it("should do nothing if user aborted the operation", async() => {
      ...
      let promiseRejecter;
      props.context.port.addRequestListener("cipherguard.resources.create", () => {
