...
class ResourceCreatePage extends React.Component {
  ...
  async save() {
    ...
    try {
      resource = await this.props.context.port.request("cipherguard.resources.create", resourceDto, secretDto);
    } catch (error) {
  ...
  handleSubmitError(error) {
    ...
    const isBadRequestError = error.name === "CipherguardApiFetchError"
      && error.data.code === 400
