...
describe("Sanitize URL checks", () => {
  it("Baseline checks", () => {
    ...
    expect(sanitizeUrl('cipherguard.com')).toBe(false);
    ...
    expect(sanitizeUrl('https://cipherguard.com')).not.toBe(false);
    expect(sanitizeUrl('https://www.cipherguard.com')).not.toBe(false);
    expect(sanitizeUrl('https://www.cipherguard.com/test')).not.toBe(false);
  });
  ...
  it("White listed protocols option", () => {
    ...
    expect(sanitizeUrl('http://cipherguard.com', {whitelistedProtocols})).toBe(false);
    expect(sanitizeUrl('https://cipherguard.com', {whitelistedProtocols})).toBe(false);
  });
