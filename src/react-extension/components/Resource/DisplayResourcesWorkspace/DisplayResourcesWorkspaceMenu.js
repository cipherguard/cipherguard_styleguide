...
class DisplayResourcesWorkspaceMenu extends React.Component {
  ...
  async handleMarkAsExpiredClick() {
    ...
    try {
      await this.props.context.port.request("cipherguard.resources.set-expiration-date", resourcesExpiryDateToUpdate);
  ...
  decryptResourceSecret() {
    return this.props.context.port.request("cipherguard.secret.find-by-resource-id", this.selectedResources[0].id);
  }
