...
class FilterResourcesByFoldersItem extends React.Component {
  ...
  handleDropEvent() {
    ...
    if (!isDroppingOnDraggedItem) {
      if (folders?.length > 0) {
        this.props.context.port.request("cipherguard.folders.move-by-id", folders[0], folderParentId);
      } else if (resources?.length > 0) {
        this.props.context.port.request("cipherguard.resources.move-by-ids", resources, folderParentId);
      }
    }
