...
class FilterResourcesByFolders extends React.Component {
  ...
  handleDropTitle() {
    ...
    if (folders?.length > 0) {
      this.props.context.port.request("cipherguard.folders.move-by-id", folders[0], null);
    } else if (resources?.length > 0) {
      this.props.context.port.request("cipherguard.resources.move-by-ids", resources, null);
    }
