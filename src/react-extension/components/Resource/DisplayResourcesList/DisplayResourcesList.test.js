...
describe("Display Resources", () => {
  ...
  describe('As LU, I should copy the secret.', () => {
    it('As LU, I should be able to copy the secret of resource', async() => {
      ...
      expect(props.context.port.request).toHaveBeenCalledWith('cipherguard.secret.find-by-resource-id', props.resourceWorkspaceContext.filteredResources[0].id);
      ...
      expect(props.context.port.request).toHaveBeenCalledWith('cipherguard.secret.find-by-resource-id', props.resourceWorkspaceContext.filteredResources[3].id);
