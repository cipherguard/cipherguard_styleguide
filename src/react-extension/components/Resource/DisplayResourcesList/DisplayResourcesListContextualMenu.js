...
class DisplayResourcesListContextualMenu extends React.Component {
  ...
  decryptResourceSecret() {
    return this.props.context.port.request("cipherguard.secret.find-by-resource-id", this.resource.id);
  }
  ...
  async handleMarkAsExpiredClick() {
    try {
      await this.props.context.port.request("cipherguard.resources.set-expiration-date", [{id: this.resource.id, expired: formatDateForApi(DateTime.utc())}]);
