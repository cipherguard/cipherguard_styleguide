...
class EditResource extends Component {
  ...
  async initializeSecret() {
    let decryptedSecret;
    try {
      decryptedSecret = await this.props.context.port.request("cipherguard.secret.find-by-resource-id", this.props.resource.id);
    } catch (error) {
      this.handleClose();
    }
  ...
  async updateResource() {
    ...
    if (!isSecretChanged) {
      await this.props.context.port.request("cipherguard.resources.update", this.state.resourceViewModel.toResourceDto(), null);
      return;
    }
    ...
    await this.props.context.port.request("cipherguard.resources.update", resourceDto, secretDto);
  }
