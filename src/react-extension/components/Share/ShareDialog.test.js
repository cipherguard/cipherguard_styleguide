...
describe("As Lu I should see the share dialog", () => {
  ...
  describe('As LU I can start sharing resources', () => {
    ...
    it('As LU I see a success toaster message after sharing resources to users and groups with success', async() => {
      ...
      expect(context.port.request).toHaveBeenCalledWith('cipherguard.resources.find-all-by-ids-for-display-permissions', shareDialogProps.resourcesIds);
      ...
      const requestBextMockImpl = (request, option) => {
        switch (request) {
          case "cipherguard.keyring.get-public-key-info-by-user":
      ...
      expect(page.aroDetails(1)).toBe('ada@cipherguard.com');
      expect(page.aroName(2)).toBe('Betty Holberton');
      expect(page.aroDetails(2)).toBe('betty@cipherguard.com');
      expect(page.aroName(3)).toBe('Board');
      expect(page.aroDetails(3)).toBe('Group');
      expect(page.aroName(12)).toBe('Admin User');
      expect(page.aroDetails(12)).toBe('admin@cipherguard.com');
      ...
      expect(context.port.request).toHaveBeenCalledWith("cipherguard.share.resources.save", shareDialogProps.resourcesIds, permissionDto);
  ...
  describe('As LU I can start sharing one folder', () => {
    ...
    it('As LU I see a success toaster message after sharing one folder to users and groups with success', async() => {
      ...
      expect(page.aroDetails(3)).toBe('adele@cipherguard.com');
      ...
      expect(context.port.request).toHaveBeenCalledWith("cipherguard.share.folders.save", shareDialogProps.foldersIds[0], permissionDto);
      expect(ActionFeedbackContext._currentValue.displaySuccess).toHaveBeenCalledWith('The permissions have been changed successfully.');
      expect(props.onClose).toBeCalled();
    });
  });
