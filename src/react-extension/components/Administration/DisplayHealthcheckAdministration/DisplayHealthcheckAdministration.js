...
class DisplayHealthcheckAdministration extends Component {
  ...
  render() {
      ...
      if (healthcheckData.ssl.peerValid === true) {
        ...
        return (
          <span className='healthcheck-fail'>
            ...
            <Tooltip message={<span>Check <a href="https://help.cipherguard.com/faq/hosting/troubleshoot-ssl" target="_blank" rel="noopener noreferrer">this guide</a></span>}>
    ...
    const isApiConfigFilePresent = () => {
      if (healthcheckData.configFile.cipherguard === true) {
        return (
          <span className='healthcheck-success'>
            <Icon name="check" />
            The cipherguard config file is present
          </span>
        );
      } else {
        return (
          <span className='healthcheck-warning'>
            <Icon name="warning" />
            The cipherguard config file is missing
            <Tooltip message={`Copy config/cipherguard.default.php to config/cipherguard.php`}>
              <Icon name='info-circle'/>
            </Tooltip>
          </span>
        );
    ...
    const whichPhpVersionIsInstalled = () => {
      if (healthcheckData.environment.info.phpVersion && healthcheckData.environment.phpVersion === true) {
        ...
        return (
          <span className='healthcheck-fail'>
            <Icon name="close" />
            PHP version is too low, cipherguard need PHP 7.4 or higher
          </span>
        );
      }
    };
    ...
    const isImagickInstalled = () => {
      if (healthcheckData.environment.image === true) {
        ...
        return (
          <span className='healthcheck-fail'>
            <Icon name="close" />
            You must enable the gd or imagick extensions to use Cipherguard
    ...
    const isIntlInstalled = () => {
      if (healthcheckData.environment.intl === true) {
        ...
        return (
          <span className='healthcheck-fail'>
            <Icon name="close" />
            You must enable the intl extension to use Cipherguard
    ...
    const isMbStringInstalled = () => {
      if (healthcheckData.environment.mbstring === true) {
        ...
        return (
          <span className='healthcheck-fail'>
            <Icon name="close"/>
            You must enable the mbstring extension to use Cipherguard
      ...
      if (healthcheckData.gpg.gpgKeyPublic === true && healthcheckData.gpg.gpgKeyPublicReadable === true && healthcheckData.gpg.gpgKeyPublicBlock) {
        ...
        return (
          <span className='healthcheck-fail'>
            ...
            <Tooltip message={`Ensure the public key file is defined by the variable cipherguard.gpg.serverKey.public in ${healthcheckData.application.configPath.toString()}`}>
              <Icon name='info-circle'/>
            </Tooltip>
          </span>
        );
      ...
      if (healthcheckData.gpg.gpgKeyPrivate === true && healthcheckData.gpg.gpgKeyPrivateReadable === true && healthcheckData.gpg.gpgKeyPrivateBlock) {
        ...
        return (
          <span className='healthcheck-fail'>
            ...
            <Tooltip message={`Ensure the private key file is defined by the variable cipherguard.gpg.serverKey.private in ${healthcheckData.application.configPath.toString()}`}>
              <Icon name='info-circle'/>
            </Tooltip>
          </span>
        );
    ...
    const isUsingLatestVersion = () => {
      if (healthcheckData.application.latestVersion === true && healthcheckData.application.info.remoteVersion) {
        return (
          <span className='healthcheck-success'>
            <Icon name="check"/>
            Using latest cipherguard version ({healthcheckData.application.info.remoteVersion.toString()})
          </span>
        );
        ...
        return (
          <span className='healthcheck-fail'>
            ...
            <Tooltip message={<span>See <a href="https://help.cipherguard.com/hosting/update" target="_blank" rel="noopener noreferrer">this guide</a></span>}>
              <Icon name='info-circle'/>
            </Tooltip>
          </span>
        );
        ...
        return (
          <span className='healthcheck-fail'>
            ...
            <Tooltip message={<span>To confirm that you are running the latest version, check <a href="https://help.cipherguard.com/releases/" target="_blank" rel="noopener noreferrer">all the releases notes</a></span>}>
              <Icon name='info-circle'/>
            </Tooltip>
          </span>
        );
    ...
    const isForceSSLEnabled = () => {
      if (healthcheckData.application.sslForce === true) {
        return (
          <span className='healthcheck-success'>
            <Icon name="check"/>
            Cipherguard is configured to force SSL use
          </span>
        );
      } else {
        return (
          <span className='healthcheck-fail'>
            <Icon name="close"/>
            Cipherguard is not configured to force SSL use
            <Tooltip message={`Set cipherguard.ssl.force to true in ${healthcheckData.application.configPath.toString()}`}>
              <Icon name='info-circle'/>
            </Tooltip>
          </span>
        );
      ...
      if (healthcheckData.application.seleniumDisabled === true) {
        ...
        return (
          <span className='healthcheck-fail'>
            ...
            <Tooltip message={`Set cipherguard.selenium.active to false in ${healthcheckData.application.configPath.toString()}`}>
              <Icon name='info-circle'/>
            </Tooltip>
          </span>
        );
      ...
      if (healthcheckData.application.robotsIndexDisabled === true) {
        ...
        return (
          <span className='healthcheck-fail'>
            ...
            <Tooltip message={`Set cipherguard.meta.robots to false in ${healthcheckData.application.configPath.toString()}`}>
              <Icon name='info-circle'/>
            </Tooltip>
          </span>
        );
    ...
    const isRegistrationPublicRemovedFromCipherguard = () => {
      if (healthcheckData.application.registrationClosed.isRegistrationPublicRemovedFromCipherguard === true) {
        return (
          <span className='healthcheck-success'>
            <Icon name="check"/>
        ...
        return (
          <span className='healthcheck-warning'>
            ...
            <Tooltip message={`You may remove the cipherguard.registration.public setting`}>
              <Icon name='info-circle'/>
            </Tooltip>
          </span>
        );
    ...
    const isJsAppServed = () => {
      if (healthcheckData.application.jsProd === true) {
        ...
        return (
          <span className='healthcheck-warning'>
            <Icon name="warning"/>
            Using non-compiled Javascript. Cipherguard will be slower
            <Tooltip message={`Set cipherguard.js.build in ${healthcheckData.application.configPath.toString()}`}>
              <Icon name='info-circle'/>
            </Tooltip>
          </span>
        );
        ...
        return (
          <>
            ...
            <div className="healthcheck-app-section">
              ...
              <div>{isRegistrationPublicRemovedFromCipherguard()}</div>
      ...
      <div className="row">
        ...
        <div className="col4 last">
          <div className="sidebar-help">
            ...
            <p><Trans>This page is available to help administrators diagnose if something is wrong with a cipherguard installation and help keeping it secure.</Trans></p>
          ...
          <div className="sidebar-help">
            ...
            <a className="button" href="https://www.cipherguard.com/docs/admin/server-maintenance/cipherguard-api-status/" target="_blank" rel="noopener noreferrer">
              <Icon name="document"/>
