...
class DisplayResourceDetailsInformation extends React.Component {
  ...
  async loadUserInformation() {
    const resourceInformation = await this.props.context.port.request("cipherguard.resources.find-details", this.resource.id);
  ...
  decryptResourceSecret(resourceId) {
    return this.props.context.port.request("cipherguard.secret.find-by-resource-id", resourceId);
  }
