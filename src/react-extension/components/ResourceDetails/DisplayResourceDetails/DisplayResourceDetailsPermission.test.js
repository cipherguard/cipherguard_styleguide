...
describe("See permissions", () => {
  ...
  describe(' As LU I can see permissions of a resource with at least one permission', () => {
    ...
    it('I should be able to identify each permission name', async() => {
      await page.title.click();
      expect(page.displayPermissionList.name(1)).toBe('Ada Lovelace (ada@cipherguard.com)');
      expect(page.displayPermissionList.name(2)).toBe('Admin User (admin@cipherguard.com)');
      expect(page.displayPermissionList.name(3)).toBe('Marketing');
      expect(context.port.request).toHaveBeenCalledWith("cipherguard.permissions.find-aco-permissions-for-display", props.resourceWorkspaceContext.details.resource.id, "Resource");
    });
