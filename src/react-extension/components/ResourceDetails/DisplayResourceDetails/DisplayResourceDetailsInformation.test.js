...
describe("DisplayResourceDetailsInformation", () => {
  ...
  beforeEach(() => {
    ...
    props.context.port.addRequestListener("cipherguard.resources.find-details", async() => resourceWithContain);
  });
  ...
  describe(' As LU I can see information of a resource', () => {
    ...
    it('I should be able to identify each information name', async() => {
      ...
      expect(page.displayInformationList.modifiedBy(1).textContent).toBe('ada@cipherguard.com');
      ...
      expect(page.displayInformationList.modifiedBy(2).textContent).toBe('ada@cipherguard.com');
  ...
  describe(' As LU I can copy a secret of a resource to clipboard', () => {
    it('AS LU, I should be able to copy the secret of a resource to clipboard', async() => {
      ...
      expect.assertions(5);
      expect(props.context.port.request).toHaveBeenCalledWith("cipherguard.secret.find-by-resource-id", props.resourceWorkspaceContext.details.resource.id);
