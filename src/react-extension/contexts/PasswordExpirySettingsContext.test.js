...
describe("PasswordExpirySettingsContext", () => {
  describe('::findSettings', () => {
    it('should not call for cipherguard.password-expiry.get-or-find if the feature flag is disabled', async() => {
      expect.assertions(1);

      const props = defaultProps();
      props.context.siteSettings.canIUse = () => false;

      props.context.port.addRequestListener('cipherguard.password-expiry.get-or-find', () => {
    ...
    it('should call for cipherguard.password-expiry.get-or-find to retrieve the settings', async() => {
      expect.assertions(1);

      const expectedSettings = defaultPasswordExpirySettingsEntityDto();

      const props = defaultProps();
      props.context.port.addRequestListener('cipherguard.password-expiry.get-or-find', () => expectedSettings);
    ...
    it('should not called twice cipherguard.password-expiry.get-or-find', async() => {
      expect.assertions(2);
  ...
  describe("::getDefaultExpirationDate", () => {
    ...
    it('should return the date from the settings', async() => {
      ...
      const props = defaultProps();
      props.context.port.addRequestListener('cipherguard.password-expiry.get-or-find', async() => expectedSettings);
