...
describe("Resource Workspace Context", () => {
  ...
  describe("As LU I should have the appropriate search filtered results at any time", () => {
    ...
    it("AS LU I should have resources belonged to a group when the filter is GROUP", async() => {
      ...
      context.port.addRequestListener("cipherguard.resources.find-all-ids-by-is-shared-with-group", async() => mockGroupResources.map(group => group.id));
      context.port.addRequestListener("cipherguard.resources.update-local-storage", async() => {
        if (page.filter.type === ResourceWorkspaceFilterTypes.GROUP) {
          throw new Error("'cipherguard.resources.update-local-storage' should have been called after filtering by GROUP");
        }
      });
  ...
  describe("As LU I be able to follow a resource uri", () => {
    it("As LU I be able to follow a safe resource uri", () => {
      ...
      expect(window.open).toHaveBeenCalledWith("https://cipherguard.com/", "_blank", "noopener,noreferrer");
    });
